...
export default function OpeningSequence({assets,onBack,backLabel="← Back",soundDevice,displayMode,initiallyMuted=false,hercules=false,directory="C:\\",initialTrack,onRolandDevice,onRolandPower,rolandPower,onRunningChange,embedded=false,autoStart=false}:{autoStart?:boolean;embedded?:boolean;assets:Assets;onBack:()=>void;backLabel?:string;soundDevice?:'pc-speaker'|'mt32'|'tandy';initiallyMuted?:boolean;hercules?:boolean;displayMode?:NativeBrowserDisplayMode;directory?:string;initialTrack?:number[];onRunningChange?:(running:boolean)=>void;rolandPower?:BrowserMt32Power;onRolandDevice?:(device:BrowserNativeMt32Device|undefined)=>void;onRolandPower?:(power:Awaited<ReturnType<typeof createBrowserNativeMt32Music>>['power']|undefined)=>void}){
 const graphics=useRef<BrowserGraphicsSwitch>({enabled:false}),[upgraded,setUpgraded]=useState(false),[graphicsNotice,setGraphicsNotice]=useState('');
 graphics.current.notice=setGraphicsNotice;
 const toggleGraphics=()=>{const enabled=!graphics.current.enabled;graphics.current.enabled=enabled;setUpgraded(enabled);setGraphicsNotice(enabled?'Upgraded graphics · experimental':'Original graphics');graphics.current.refresh?.();if(started)focusBrowserGameCanvas(canvas.current!);};
 ...
 return <section ref={surface} className={embedded?"launcher-game-session":undefined}><div className="game-toolbar"><button onClick={onBack}>{backLabel}</button><button aria-pressed={upgraded} onClick={toggleGraphics}>{upgraded?"Use original graphics":"Enable graphics update"}</button><span className="sr-only" role="status">{status}</span>{embedded&&<button onClick={()=>void toggleFullscreen()}>{fullscreen?"Exit full screen":"Full screen"}</button>}{!started&&!embedded&&<button onClick={()=>{audioContext.current=new AudioContext();void audioContext.current.resume();setStarted(true);}}>Play Stunts</button>}</div><output className="sr-only">{graphicsNotice||"Original graphics"}</output><div className={embedded?"launcher-screen":undefined}><canvas ref={canvas} width={1280} height={800} tabIndex={0} aria-label="Native Stunts opening, menus, races and track editor" style={{width:embedded?'100%':'min(100%, calc((100dvh - 220px) * 4 / 3))',height:'auto',aspectRatio:'4 / 3',margin:'0 auto',display:'block',background:'#000',imageRendering:'auto'}}/>{embedded&&!started&&<div className="launcher-screen-idle stunts-game-idle"><img className="stunts-idle-art" src="/intro-art-review/main-menu-original-320x200.png" alt=""/><div className="stunts-idle-shade"/><button className="launcher-play" onClick={()=>{audioContext.current=new AudioContext();void audioContext.current.resume();setStarted(true);}}>Play Stunts</button></div>}</div>{fullscreenError&&<p role="status">{fullscreenError}</p>}{error&&<p role="alert">{error}</p>}</section>;
}
