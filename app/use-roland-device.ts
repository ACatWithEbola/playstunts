...
export function useRolandDevice(){
 ...
 useEffect(()=>{let disposed=false;const owned=createBrowserMt32Power();const unsubscribe=owned.subscribe(setDevice);setPower(owned);setPowerBusy(true);void owned.powerOn().catch(error=>{if(!disposed)setPowerNotice(String(error));}).finally(()=>{if(!disposed)setPowerBusy(false);});return()=>{disposed=true;unsubscribe();owned.close();};},[]);
