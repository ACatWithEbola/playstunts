...
export function controlAllocatedPcSpeakerDialogAudio(memory:Uint8Array,d:number,driverSegment:number,operation:'pause-audio'|'resume-audio',port61:number){
 const v=new DataView(memory.buffer,memory.byteOffset,memory.byteLength),word=(at:number)=>v.getUint16(d+at,true);
 if(memory[d+0x4e06])throw Error('Allocated dialog audio requires regular PC15 mode');
 const driver=memory.slice(driverSegment*16,driverSegment*16+0x2df);
 ...
 for(const effect of effects){
  if(effect.type==='volume'){
   ...
   for(let index=0;index<memory[d+0x9fe4];index++){
    const at=d+0xa036+index*46;if(memory[at]===effect.owner)setOriginalPcSpeakerVolume(driver,index,effect.value);
   }
  }else if(effect.type==='update-voice'){
   const at=d+0xa036+effect.index*46,channel=memory[at+44];
   if(memory[at+1]){const timer=d+v.getUint16(at+42,true),record=memory.slice(at,at+46);updateOriginalPcSpeakerVoice(driver,channel,record,memory.slice(timer,timer+72),instrument(at));memory.set(record,at);}
  }else if(effect.type==='flush'){const next=stepOriginalPcSpeakerOutput(driver,port61);writes.push(...next.writes);for(const [port,value] of next.writes)if(port===0x61)port61=value;}
 ...
 memory.set(driver,driverSegment*16);return writes;
}
