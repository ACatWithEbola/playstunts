...
import {setOriginalMt32EffectsEnabled} from './mt32-effect-mute.ts';
import {exitOriginalMt32RaceAudio} from './mt32-race-audio-exit.ts';
...
import {stepOriginalMt32CarInterrupt} from './mt32-engine-interrupt.ts';
import {updateOriginalMt32DrivingSounds} from './mt32-driving-sound-runtime.ts';
...
export type OriginalMt32RaceState=Omit<OriginalMt32EngineState,'car'>&{cars:Uint8Array[];markers:Uint8Array;busy:number[];carCounter:number;soundFlags:number[]};
 ...
 */
export function createOriginalMt32RaceAudio(before:OriginalMt32RaceState,resources:LoadedEffectResource[],enabled=true,master=127){
 let state=structuredClone(before);
 const output=(writes:number[][])=>writes;
 ...
 const apply=(handle:number,next:Omit<OriginalMt32EngineState,'command'>&{command?:Uint8Array;writes:number[][]})=>{
 ...
 return {
  setEnabled(value:boolean,savedVolumes:Uint8Array){
   const next=setOriginalMt32EffectsEnabled({...state,enabled:enabled?1:0,savedVolumes},value);
  ...
  start(handle:number){const car=carAt(handle);return apply(handle,startOriginalMt32Engine({...state,car},instrumentAt(car)));},
  ...
  skid(handle:number,variant:1|2|'stop'){return apply(handle,updateOriginalMt32Skid({...state,car:carAt(handle)},variant,resources,enabled,master));},
  driveSounds(handle:number,flags:number){
   const next=updateOriginalMt32DrivingSounds({...state,car:carAt(handle),soundFlags:state.soundFlags[handle]??0},flags,resources,enabled,master);
  ...
  tick(){
   const next=stepOriginalMt32CarInterrupt({...state,car:carAt(0)},resources,enabled,master);
   const {writes,...retained}=next;state={...state,...retained};return output(writes);
  },
  exit(queue:DrivingAudioExitState){
   const {audio,queue:nextQueue}=exitOriginalMt32RaceAudio({...state,paused:0},queue,resources,enabled,master);
  ...
  dispatchRequests(requests:readonly RaceAudioRequest[],soundFlags:readonly number[]){
   ...
   for(const request of requests){
    if(request.kind==='reset'){
     const {writes:resetWrites,...next}=resetOriginalMt32Audio({...state,paused:0});state={...state,...next};writes.push(...output(resetWrites));continue;
    }
    ...
    if(request.kind==='engine-start')writes.push(...apply(handle,startOriginalMt32Engine({...state,car},instrumentAt(car))));
    else if(request.kind==='engine-stop')writes.push(...apply(handle,stopEngineRuntime({...state,car})));
    else writes.push(...apply(handle,updateOriginalMt32Skid({...state,car},request.kind==='skid-start'?1:request.kind==='skid2-start'?2:'stop',resources,enabled,master)));
   }
