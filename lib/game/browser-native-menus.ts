...
export async function createBrowserNativeMenus(options:BrowserNativeMenuOptions){
 ...
 const car=async(config:number[],opponent:number)=>{
  ...
  if(!options.displayMode){
   ...
   try{return await runNativeCarMenu(carHost);}finally{showroom?.close();if(options.graphics?.refresh===presentCar)options.graphics.refresh=undefined;}
  }
