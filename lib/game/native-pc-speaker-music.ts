...
export async function createNativePcSpeakerMusic(context:AudioContext){
 ...
 return {control,get settings(){return {musicEnabled,soundEnabled,paused};},play(name:NativeMusicScore){if(closed||!musicEnabled)return;const restorePaused=paused,restoreSound=soundEnabled;stop();const seed=seeds[nativeMusicScores.indexOf(name)];runtime=createControlledOriginalPcSpeakerMusic(seed);stream=createOriginalPcSpeakerMusicStream(runtime,seed.initialWrites,context.sampleRate,speaker);if(!restoreSound)control('toggle-sound');if(restorePaused)control('pause-audio');pump();timer=setInterval(pump,25);},
