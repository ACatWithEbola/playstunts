...
export function updateOriginalMt32Skid<T extends CarEffectState&OriginalMt32EffectState>(before:T,variant:1|2|'stop',resources:LoadedEffectResource[],enabled:boolean,master:number){
 ...
 const car=skidAudio(before.car,variant,args=>{
  ...
  const stopped=stopOriginalMt32Effect(state,handle,master);state={...state,...stopped};writes.push(...stopped.writes);
 });
