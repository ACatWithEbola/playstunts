...
export type OriginalPcSpeakerMemoryState=OriginalPcSpeakerRaceState&{driverSegment:number};
function view(memory:Uint8Array,d:number,driverSegment:number){
 ...
 if(!Number.isInteger(driverSegment)||driverSegment<0||driverSegment>65535||driverSegment*16+0x2df>memory.length)throw Error('Original PC15 driver is outside memory');
 return new DataView(memory.buffer,memory.byteOffset,memory.byteLength);
}
/** Read the allocated car records and shared PC15 tables in place.
 * The caller supplies the loaded driver segment; no captured startup state is copied. */
export function readOriginalPcSpeakerRaceState(memory:Uint8Array,d:number,driverSegment:number):OriginalPcSpeakerMemoryState{
 ...
 return {cars:records(0x618e,25,76),timers:records(0x801e,24,72),voices:records(0xa036,5,46),command:memory.slice(d+0x70c4,d+0x70ce),lastNotes:memory.slice(d+0x90e0,d+0x90f8),markers:memory.slice(d+0x931a,d+0x9332),busy:Array.from(memory.slice(d+0xa3aa,d+0xa3c2)),driver:memory.slice(driverSegment*16,driverSegment*16+0x2df),carCounter:word(0x3340),driverSegment,soundFlags};
}
...
export function writeOriginalPcSpeakerRaceState(memory:Uint8Array,d:number,state:OriginalPcSpeakerMemoryState){
 const v=view(memory,d,state.driverSegment);
 const tables:[[Uint8Array[],number,number,number],[Uint8Array[],number,number,number],[Uint8Array[],number,number,number]]=[[state.cars,0x618e,25,76],[state.timers,0x801e,24,72],[state.voices,0xa036,5,46]];
 ...
 for(const [bytes,size] of [[state.command,10],[state.lastNotes,24],[state.markers,24],[state.busy,24],[state.driver,0x2df]] as const)if(bytes.length!==size)throw Error('Incomplete original audio state');
