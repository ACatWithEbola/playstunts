...
export function updateOriginalPcSpeakerSkid<T extends CarEffectState&OriginalPcSpeakerEffectState>(before:T,variant:1|2|'stop',resources:LoadedEffectResource[],enabled:boolean,master:number){
 let state:T={...before};const writes:number[][]=[];
 const car=skidAudio(before.car,variant,args=>{
  ...
  const stopped=stopOriginalPcSpeakerEffect(state,handle,master);state={...state,...stopped};writes.push(...stopped.writes);
 });
