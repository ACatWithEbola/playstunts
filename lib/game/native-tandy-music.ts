...
export async function createNativeTandyMusic(context:AudioContext){
 ...
 return {control,get settings(){return {musicEnabled,soundEnabled,paused};},play(name:NativeMusicScore){if(closed||!musicEnabled)return;const restorePaused=paused,restoreSound=soundEnabled;stop();const seed=seeds[nativeMusicScores.indexOf(name)];runtime=createControlledOriginalTandyMusic(seed);stream=createOriginalTandyToneStream({tick:()=>runtime!.tick(0)},seed.initialWrites,context.sampleRate,options,speaker);if(!restoreSound)control('toggle-sound');if(restorePaused)control('pause-audio');pump();timer=setInterval(pump,25);},
