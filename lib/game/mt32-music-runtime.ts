...
export function createOriginalMt32MusicRuntime(seed:OriginalMt32MusicSeed){
 ...
 function sequence(owner:number){
  ...
  while(v.getUint32(0,true)&&!v.getUint32(0x18,true)){
   ...
   else if(op===0xe5){const value=c.duration??0;let low=value&255;if(value&256)low|=128;v.setInt16(0x26,((value&0xff00)>>>1)+(low<<24>>24)-8192,true);emit('bend',timer[0x47],emptyVoice,emptyPatch,v.getInt16(0x26,true));}
