...
export interface OriginalPcSpeakerEffectMuteState extends OriginalPcSpeakerEffectState {enabled:number;savedVolumes:Uint8Array}
/** Original effect mute/unmute 0x294e6/0x294b2, PC15 mode.
 * This changes logical gains while leaving sequencing and voice state intact.
 */
export function setOriginalPcSpeakerEffectsEnabled(before:OriginalPcSpeakerEffectMuteState,enabled:boolean){
 ...
 for(let owner=16;owner<24;owner++){
  ...
  for(let index=0;index<before.voices.length;index++){
   ...
   setOriginalPcSpeakerVolume(driver,index,volume);
  }
 }
