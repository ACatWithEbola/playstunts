...
function attachManualRaceRuntime(data:Parameters<typeof createNativeManualRaceSession>[0],prepared:Awaited<ReturnType<typeof createNativeManualRaceSession>>){
 ...
 let graphicsSource:Uint8Array|undefined,graphicsLive:Uint8Array|undefined,pendingGraphics:Uint8Array|undefined;
 ...
 return {...prepared,audio,
  ...
  graphicsFrame(){if(pendingGraphics){const snapshot=pendingGraphics;pendingGraphics=undefined;renderer.render(snapshot,undefined,undefined,undefined,true,(memory,world)=>{rendering=memory;graphicsSource=memory.slice();graphicsLive=snapshot.slice();drawNativeFullRedrawRaceLayers(memory,snapshot,d,bp,world);});}if(!graphicsSource||!graphicsLive||!renderer.view)return;return {...renderer.view,mask:createRaceOverlayMask(graphicsSource,graphicsLive,renderer.view.rectangle,renderer.view.fireballMask),memory:graphicsLive,pixels:rendering!.subarray(0xa0000,0xa0000+64000)};},
  ...
  renderCockpitWorld(){prepareOriginalRaceViewport(session.state.memory,d,bp);if(display){if(captureGraphics)pendingGraphics=session.state.memory.slice();const pixels=display.render(session.state.memory);displayRendered=true;return pixels;}return renderer.render(session.state.memory,undefined,undefined,undefined,captureGraphics,(memory,world)=>{rendering=memory;if(captureGraphics){graphicsSource??=new Uint8Array(memory.length);graphicsLive??=new Uint8Array(session.state.memory.length);graphicsSource.set(memory);graphicsLive.set(session.state.memory);}drawNativeFullRedrawRaceLayers(memory,session.state.memory,d,bp,world);});},
