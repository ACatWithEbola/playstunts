...
import {prepareNativeDisplayTitleCards} from './native-display-title-cards.ts';
...
export async function runBrowserNativeTitleCards(canvas:HTMLCanvasElement,mode:NativeBrowserDisplayMode,signal:AbortSignal,hercules=false,onFinish?:(display:BrowserOpeningDisplay)=>void){
 const presentHercules=hercules?createBrowserHerculesPresenter(canvas):undefined;
 const context=canvas.getContext('2d')!,surface=document.createElement('canvas');surface.width=320;surface.height=200;const drawing=surface.getContext('2d')!,image=drawing.createImageData(320,200),aborted=()=>{if(signal.aborted)throw new DOMException('Native title cards closed','AbortError');};
 const input=createBrowserMenuInput(canvas,{onPoll:aborted});const closeInput=()=>input.close();signal.addEventListener('abort',closeInput,{once:true});let display:Awaited<ReturnType<typeof prepareNativeDisplayTitleCards>>|undefined;
 ...
 try{
  display=await prepareNativeDisplayTitleCards(mode,await loadBrowserNativeInitialDisplayData(mode,hercules),await loadBrowserOriginalResourceCatalog(),present);aborted();focusBrowserGameCanvas(canvas);let previous=input.counter();
  ...
  const flow=display.titles();let step=flow.next();while(!step.done){aborted();step=flow.next(await timed(step.value.type==='present'?display.presentation(step.value.argument):originalInputWait(step.value.argument)));}onFinish?.({owner:display.owner,present});return step.value;
