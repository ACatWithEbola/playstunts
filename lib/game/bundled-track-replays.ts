...
export function bundledTrackReplays(tracks:Assets['tracks'],binary:(path:string)=>Promise<Uint8Array>,replays?:Assets['replays']){
 ...
 for(const replay of replays??[{name:'DEFAULT',file:'DEFAULT.RPL',bytes:0,sha256:''},{name:'CTKFIN',file:'CTKFIN.RPL',bytes:0,sha256:''},{name:'Flaute1',file:'Flaute1.RPL',bytes:0,sha256:''}])files.set(nativeFileKey('',replay.name,'.rpl'),()=>binary('replays/'+replay.file));
 return files;
}
