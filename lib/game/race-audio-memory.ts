...
function view(memory:Uint8Array,d:number,driverSegment:number){
 ...
 if(!Number.isInteger(driverSegment)||driverSegment<0||driverSegment>65535||driverSegment*16+0x960>memory.length)throw Error('Original AdLib driver is outside memory');
 return new DataView(memory.buffer,memory.byteOffset,memory.byteLength);
}
/** Read the allocated car records and shared regular AdLib tables in place.
 * The caller supplies the loaded driver segment; no captured startup state is copied. */
export function readOriginalRaceAudioState(memory:Uint8Array,d:number,driverSegment:number):RaceAudioState{
 ...
 return {cars:records(0x618e,25,76),timers:records(0x801e,24,72),voices:records(0xa036,10,46),command:memory.slice(d+0x70c4,d+0x70ce),lastNotes:memory.slice(d+0x90e0,d+0x90f8),markers:memory.slice(d+0x931a,d+0x9332),busy:Array.from(memory.slice(d+0xa3aa,d+0xa3c2)),velocities:Array.from(memory.slice(driverSegment*16+0x957,driverSegment*16+0x960)),carCounter:word(0x3340),driverSegment,soundFlags};
}
...
export function writeOriginalRaceAudioState(memory:Uint8Array,d:number,state:RaceAudioState){
 const v=view(memory,d,state.driverSegment);
 const tables:[[Uint8Array[],number,number,number],[Uint8Array[],number,number,number],[Uint8Array[],number,number,number]]=[[state.cars,0x618e,25,76],[state.timers,0x801e,24,72],[state.voices,0xa036,10,46]];
 ...
 for(const [bytes,size] of [[state.command,10],[state.lastNotes,24],[state.markers,24],[state.busy,24],[state.velocities,9]] as const)if(bytes.length!==size)throw Error('Incomplete original audio state');
 ...
 memory.set(state.velocities,state.driverSegment*16+0x957);v.setUint16(d+0x3340,state.carCounter&65535,true);
}
