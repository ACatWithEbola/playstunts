...
export function createMt32WasmOutput(module:Mt32Wasm,control:Uint8Array,pcm:Uint8Array):Mt32StereoOutput & {resetGeneration():number;resetControllers():void;active():boolean;panelWrite(writes:number[][]):void;unitID():number;setUnitID(value:number):void;sound(group:number,number:number):{group:string;name:string};patchName(part:number):string;read(address:number,length:number):Uint8Array;mainDisplay():void;settings():number[];set(id:number,value:number):void;display():{text:string;midi:boolean};close():void}{
 ...
 return {sampleRate,
  ...
  sound(group,number){if(closed||!Number.isInteger(group)||!Number.isInteger(number)||group<0||group>3||number<0||number>127||module._stunts_mt32_sound(group,number,midi))throw Error('Invalid Roland sound');const text=(at:number,n:number)=>new TextDecoder().decode(module.HEAPU8.subarray(midi+at,midi+at+n)).split('\0',1)[0];return {group:text(0,7),name:text(8,10)};},
  patchName(part){if(closed||!Number.isInteger(part)||part<0||part>8||module._stunts_mt32_patch_name(part,midi))throw Error('Invalid Roland part');return new TextDecoder().decode(module.HEAPU8.subarray(midi,midi+10)).split('\0',1)[0];},
